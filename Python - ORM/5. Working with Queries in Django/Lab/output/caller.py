...
# 1.	Books Finder
def find_books_by_genre_and_language(genre, book_language):
    return Book.objects.filter(genre=genre, language=book_language)
...
def find_authors_nationalities():
    authors_with_defined_nationality = Author.objects.exclude(nationality__isnull=True)

    result = [f"{str(author)} is {author.nationality}"
              for author in authors_with_defined_nationality]
...
def order_books_by_year():
    ...
    result = [f"{book.publication_year} year: {str(book)}" for book in ordered_books]
...
def delete_review_by_id(review_id):
    try:
        record = Review.objects.get(pk=review_id)
        reviewer_info = str(record)
        record.delete()
        return f"{reviewer_info} was deleted"
...
def filter_authors_by_birth_year(year_1, year_2):
    # start = date(year_1, 1, 1)
    # end = date(year_2, 12, 31)
    #
    # selected_authors = Author.objects.filter(birth_date__range=(start, end)).order_by("-birth_date")

    selected_authors = Author.objects.filter(birth_date__year_range=(year_1, year_2)).order_by("-birth_date")

    result = [f"{author.birth_date}: {str(author)}" for author in selected_authors]
    return "\n".join(result)
