...
class F1SeasonApp:
    ...
    def get_race_results(self, race_name: str, red_bull_pos: int, mercedes_pos: int) -> str:
        ...
        rev_mercedes = self.mercedes_team.calculate_revenue_after_race(mercedes_pos)
        rev_red_bull = self.red_bull_team.calculate_revenue_after_race(red_bull_pos)

        return f"Red Bull: {rev_red_bull}. Mercedes: {rev_mercedes}. {better_team} is ahead at the {race_name} race."
